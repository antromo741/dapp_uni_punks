...
describe('NFT', () => {
  ...
  describe('Deployment', () => {
    ...
    beforeEach(async () => {
      ...
      nft = await NFT.deploy('Dapp Punks', 'DP', COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
    })
    ...
    it('has correct cost to mint', async () => {
  ...
  describe('Minting', () => {
    ...
    describe('Failure', () => {
      ...
      it('rejects mint when supply is more than the max supply', async () => {
        const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10)
        const NFT = await ethers.getContractFactory('NFT')
        nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
      ...
      it('does not retrun URI for invalid token', async () => {
        const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10)
        const NFT = await ethers.getContractFactory('NFT')
        nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
  ...
  describe('Display NFTS', () => {
    ...
    describe('Success', () => {
      ...
      it('sends fungs to the owner', async () => {
